...
AS
BEGIN
    UPDATE A SET A.CCMensual = 0 FROM EFNI.dbo.Persona A
END
GO
...
BEGIN
    ...
    BEGIN TRY
        DECLARE @Lote AS INT
        SET @Lote = ISNULL((SELECT MAX(a.Lote)
    FROM EFNI.dbo.Telefono A),0);
        SET @Lote = @Lote + 1;
        SELECT @Lote
        ...
        (EFNI_Telefono,CalledCount,FechaLlamada,Tipificacion,Lote)
    SELECT
